...
Copy image, share image, and download PNG use an offscreen card renderer, so long cards are exported from the full post model instead of only the visible screen area. Image media is rendered with its real aspect ratio instead of being cropped into a fixed landscape frame. Video previews play looping MP4 variants muted by default with a sound toggle when X exposes one. Video posts switch the primary action to video download, show a Material processing state during generation, and export a short 12 fps MP4 Quoti card with the source audio into `Movies/Quoti`; PNG export still renders video posts from the poster image.
