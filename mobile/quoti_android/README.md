...
Copy image, share image, and download PNG use an offscreen card renderer, so long cards are exported from the full post model instead of only the visible screen area. Image media is rendered with its real aspect ratio instead of being cropped into a fixed landscape frame. Video previews play muted looping MP4 variants when X exposes one; PNG export still renders video posts from the poster image, and MP4 video-card export remains a follow-up.
